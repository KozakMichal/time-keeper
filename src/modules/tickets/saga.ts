...
function* handleRemove(action: ReturnType<typeof removeTicket>) {
    ...
    //if (!ticketToRemove.times) {
    //    return console.error('No times - cannot end!');
    //}

    //if (ticketToRemove.times[ticketToRemove.times.length - 1].end) {
    //    return console.error('Cannot end already finished time!');
    //}

    //ticketToRemove.times[ticketToRemove.times.length - 1].end = moment().utc().valueOf();
    ...
    //yield put(ticketChanged(ticketToRemove));
}
